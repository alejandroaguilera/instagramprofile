...
- **Tema claro/oscuro**: todos los colores de UI salen de tokens semánticos (`--ink`…`--ink-4`, `--surface`…`--surface-3`, `--bg`, `--hairline`, `--hairline-strong`, `--line`, `--hover`, `--nav-glass`, sombras). El dark redefine los tokens bajo `:root[data-theme="dark"]` en `index.css` — no hay overrides por componente (única excepción: `.notif-icon-follow`). Un script inline en `index.html` fija `data-theme` antes del primer paint (localStorage `theme` o `prefers-color-scheme`); el toggle vive en el header (`App.jsx`) y solo persiste en localStorage cuando el usuario lo pulsa — sin elección explícita se sigue al sistema. Se quedan hardcodeados a propósito: gradientes (stories/reel/fallback de avatar/ring de story), blancos sobre media o sobre `--accent`, overlays y el rosa de marca.
...
- **2026-07-19** — Dark mode: tokens semánticos para todos los colores de UI (los ~146 valores sueltos de los CSS migrados a `var(--…)`), variante oscura vía `:root[data-theme="dark"]`, toggle sol/luna en el header, arranque según `prefers-color-scheme` con script anti-flash en `index.html`, persistencia en localStorage solo tras elección explícita. Los mockups también se oscurecen (equivale a previsualizar la foto en el dark mode de Instagram).
